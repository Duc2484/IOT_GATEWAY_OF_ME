    ...
    ui(new Ui::MainWindow)
{
     ...
     */
    QImage image("/home/nguyendangchungduc/Source_Code/QT_Aplications/GATEWAY/build/WSAN/logo.png");
    ...
    QFile file("./WSAN/AIDustDensity.txt");
        ...
        if (file.open(QIODevice::ReadOnly | QIODevice::Text | QIODevice::ReadWrite)) {
            ...
            while (!stream.atEnd()) {
                ...
                tmp = "\n["+QTime::currentTime().toString()+ "] \n";
                console->insertPlainText(tmp);
                double OT = rand() % 50 + 100;
                double OP = rand() % 50 + 100;
                double H2S = rand() % 30 + 10;
                double ND = rand() % 10 + 1;
                sendMqttData(mac, dus, "Value Dust");
                sendMqttData(mac, OT, "Value MaxOT");
                sendMqttData(mac, OP, "Value MaxOP");
                sendMqttData(mac, H2S, "H2SContent");
                sendMqttData(mac, ND, "NormalDiameter");
